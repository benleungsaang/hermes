...
2. **Using `write_file` to "add a section" to an existing user-workspace file (`AGENTS.md`, `reminders.md`, `corrections.md`, `HOT.md`, etc.).** `write_file` is **full overwrite**, not append. A 5-line intent to "add a new section to AGENTS.md" can silently destroy all existing sections if the agent forgets to re-read the file first. **Default to `patch`** for any modification to an existing markdown file. If a rewrite is genuinely required (full restructure, format migration), read the file first, build the complete new content, then `write_file` — never from memory alone. Verified failure: 2026-06-17, the agent wrote a 64-line replacement of `AGENTS.md` and silently lost 163 lines of pre-existing rules (Reminders分类, 记忆助手工作流, 微信写作规范). Recovery: rewrite the full file from memory and verify with `grep -c "^##" file.md`.
