...
**Beyond inbound tool output:** redaction also fires on outbound HTTP from inside the sandbox — `urllib`, `http.client`, `requests`, and env-var substitution in `subprocess.run(env=...)` all get keys replaced with stars on the wire. Symptom: a Python process prints the full key (looks fine) but the remote API returns `401 invalid api key: ****X***`. Workaround: write the literal key to a `.sh` file, then invoke it via `subprocess.run(['/bin/bash', '/tmp/x.sh'])` (argv, not env). The bash process reads the file directly — that path is not redacted. Full reproduction + verification recipe: `references/sandbox-secret-redaction.md` in `hermes-secondary-model-on-demand`.
